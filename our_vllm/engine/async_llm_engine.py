...
from our_vllm.config import (DecodingConfig, EngineConfig, LoRAConfig, ModelConfig,
                         ParallelConfig, SchedulerConfig)
from our_vllm.core.scheduler import SchedulerOutputs
from our_vllm.engine.arg_utils import AsyncEngineArgs
from our_vllm.engine.async_timeout import asyncio_timeout
from our_vllm.engine.llm_engine import LLMEngine
from our_vllm.engine.metrics import StatLoggerBase
from our_vllm.executor.executor_base import ExecutorAsyncBase
from our_vllm.executor.ray_utils import initialize_ray_cluster, ray
from our_vllm.inputs import LLMInputs, PromptInputs
...
from our_vllm.sequence import ExecuteModelRequest, SamplerOutput
from our_vllm.usage.usage_lib import UsageContext
...
class AsyncLLMEngine:
    ...
    def _get_executor_cls(
        ...
        if isinstance(distributed_executor_backend, type):
            ...
            from our_vllm.executor.neuron_executor import NeuronExecutorAsync
            executor_class = NeuronExecutorAsync
        elif engine_config.device_config.device_type == "tpu":
            if distributed_executor_backend == "ray":
                initialize_ray_cluster(engine_config.parallel_config)
                from our_vllm.executor.ray_tpu_executor import RayTPUExecutorAsync
                executor_class = RayTPUExecutorAsync
            else:
                assert distributed_executor_backend is None
                from our_vllm.executor.tpu_executor import TPUExecutorAsync
                executor_class = TPUExecutorAsync
        elif engine_config.device_config.device_type == "cpu":
            from our_vllm.executor.cpu_executor import CPUExecutorAsync
            executor_class = CPUExecutorAsync
        elif engine_config.device_config.device_type == "openvino":
            assert distributed_executor_backend is None, (
                "Distributed execution is not supported with "
                "the OpenVINO backend.")
            from our_vllm.executor.openvino_executor import OpenVINOExecutorAsync
            executor_class = OpenVINOExecutorAsync
        elif engine_config.device_config.device_type == "xpu":
            if distributed_executor_backend is None:
                from our_vllm.executor.xpu_executor import XPUExecutorAsync
                executor_class = XPUExecutorAsync
            elif distributed_executor_backend == "ray":
                initialize_ray_cluster(engine_config.parallel_config)
                from our_vllm.executor.ray_xpu_executor import RayXPUExecutorAsync
            ...
            initialize_ray_cluster(engine_config.parallel_config)
            from our_vllm.executor.ray_gpu_executor import RayGPUExecutorAsync
            ...
            from our_vllm.executor.gpu_executor import GPUExecutorAsync
    ...
    def from_engine_args(
        ...
        if engine_args.engine_use_ray:
            from our_vllm.executor import ray_utils
